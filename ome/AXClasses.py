...
class BaseAXUIElement(_a11y.AXUIElement):
    """
    Base class for UI elements. Implements:
    1. Factory class methods for getting app/system objects.
    ...
    def getAppRefByBundleId(cls, bundleId):
        ...
        try:
            ...
            if 'Specified bundle ID not found in running apps' in str(e):
                print(f"App with bundle ID {bundleId} not found. Launching it...")
                # Use the built-in launch method instead of subprocess
                cls.launchAppByBundleId(bundleId)
                import time
                time.sleep(2)  # Wait for app to launch
                # Retry getting the app reference
                apps = ra.runningApplicationsWithBundleIdentifier_(bundleId)
                if len(apps) == 0:
                    raise ValueError(('Specified bundle ID not found in '
                                      'running apps: %s' % bundleId))
