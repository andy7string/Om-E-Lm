...
class Core(ComboBox, Menu, Mouse, PageTabList, Text, Table, Value, Generic):
    """
    Main automation interface for LDTP daemon.
    Provides methods for app/window/object interaction, wait helpers, and more.
    Inherits from ComboBox, Menu, Mouse, PageTabList, Text, Table, Value, Generic.
    """
    def __init__(self):
    ...
    def appundertest(self, app_name):
        """
        Set the application under test by bundle identifier.
        @param app_name: Application bundle identifier (e.g., 'com.apple.finder')
        @return: 1 on success
        """
    ...
    def launchapp(self, cmd, args = [], delay = 0, env = 1, lang = "C"):
        ...
        """
        try:
            ome.NativeUIElement.launchAppByBundleId(cmd)
            return 1
        except RuntimeError:
            if ome.NativeUIElement.launchAppByBundlePath(cmd, args):
