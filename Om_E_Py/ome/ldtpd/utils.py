...
def normalized_match(pattern, string, case_insensitive=True):
    """
    Symmetrically strip special characters and perform glob and substring matching.
    """
    def normalize(s):
        # Remove spaces, colons, dots, underscores, newlines
        return re.sub(r'[\s:._]', '', s)
    p_norm = normalize(pattern)
    s_norm = normalize(string)
    if case_insensitive:
        p_norm = p_norm.lower()
        s_norm = s_norm.lower()
    # Try glob match
    if fnmatch.fnmatchcase(s_norm, p_norm):
        return True
...
class Utils(object):
    ...
    def _running_apps(self):
        if not Utils._singleton_running_apps:
            apps = ome.NativeUIElement._getRunningApps()
    ...
    def _get_front_most_window(self):
        app=ome.NativeUIElement.getFrontmostApp()
        return app.windows()[0]

    def _get_any_window(self):
        front_app=ome.NativeUIElement.getAnyAppWithWindow()
    ...
    def _glob_match(self, pattern, string):
        """
        Symmetric, case-insensitive glob and substring match.
        """
        return normalized_match(pattern, string)

    def _match_name_to_appmap(self, name, acc):
        if not name:
            return 0
        # Try direct match on obj_index and label
        if self._glob_match(name, acc['obj_index']):
            return 1
        if self._glob_match(name, acc['label']):
            return 1
        # Try normalized match
        if normalized_match(name, acc['label']):
            return 1
        return 0
        ...
        for gui in set(self._running_apps):
            ...
            try:
                ...
                if not app_windows and app.AXRole == "AXApplication":
                    ...
                    windows[key]["app"]=app
                    continue
            except (ome._a11y.ErrorAPIDisabled, \
                        ome._a11y.ErrorCannotComplete, \
                        ome._a11y.Error, \
                        ome._a11y.ErrorInvalidUIElement):
                pass
    ...
    def _get_title(self, obj):
        ...
        try:
            ...
            if checkBox:
                ...
                try:
                    title=obj.AXHelp
                except (ome._a11y.ErrorUnsupported, ome._a11y.Error):
                    pass
            if not title:
                title=obj.AXTitle
        except (ome._a11y.ErrorUnsupported, ome._a11y.Error):
    ...
    def _update_apps(self):
        ...
        self._running_apps=ome.NativeUIElement._getRunningApps()
    ...
    def _get_object_handle(self, window_name, obj_name, obj_type=None,
        ...
        try:
            return self._internal_get_object_handle(window_name, obj_name,
                                                    ...
                                                    force_remap)
        except ome._a11y.ErrorInvalidUIElement:
    ...
    def _internal_get_object_handle(self, window_name, obj_name, obj_type=None,
        ...
        except (ome._a11y.ErrorCannotComplete,
                ome._a11y.ErrorUnsupported,
                ome._a11y.ErrorInvalidUIElement, AttributeError):
            # During the test, when the window closed and reopened
