...
from ApplicationServices import (
    AXUIElementGetTypeID, AXUIElementCopyAttributeNames, AXUIElementCopyAttributeValue,
    AXUIElementSetAttributeValue, AXUIElementPerformAction, AXUIElementCopyActionNames,
    AXUIElementIsAttributeSettable, AXUIElementCopyElementAtPosition, AXUIElementGetPid,
    AXUIElementCreateApplication, AXUIElementCreateSystemWide, AXObserverCreate,
    AXObserverAddNotification, AXObserverRemoveNotification, AXObserverGetRunLoopSource,
    AXValueGetType, kAXValueCGSizeType, kAXValueCGPointType, kAXValueCFRangeType,
    kAXErrorSuccess, kAXErrorAttributeUnsupported, kAXErrorActionUnsupported,
    kAXErrorNotificationUnsupported, kAXErrorAPIDisabled, kAXErrorInvalidUIElement,
    kAXErrorCannotComplete, kAXErrorNotImplemented, kAXErrorNoValue, kAXErrorIllegalArgument
)
from Quartz import CFNumberGetValue, CFGetTypeID, CFStringGetTypeID, CFBooleanGetTypeID, CFArrayGetTypeID, CFNumberGetTypeID, CFEqual
from Quartz import kCFNumberIntType, kCFNumberDoubleType
from Quartz import CFRunLoopAddSource, CFRunLoopGetCurrent, kCFRunLoopDefaultMode
from Quartz import NSSizeFromString, NSPointFromString, NSRangeFromString
from ApplicationServices import AXIsProcessTrusted

# Load AXUIElementSetMessagingTimeout if available
AXUIElementSetMessagingTimeout = None
try:
    functions = [
        ("AXUIElementSetMessagingTimeout", b"i^{__AXUIElement=}d"),
    ]
    objc.loadBundleFunctions(
        objc.loadBundle("ApplicationServices", globals(), bundle_path="/System/Library/Frameworks/ApplicationServices.framework"),
        globals(),
        functions
    )
except Exception as e:
    print("Could not load AXUIElementSetMessagingTimeout:", e)
...
def _setError(error_code, error_message):
    ...
    if error_code in error_mapping:
        ...
        raise Error(f"Unknown AX error code {error_code}: {msg}")
...
def performAction(element, action):
    """Perform the specified accessibility action on the given element."""
    err = AXUIElementPerformAction(element, action)
    if err != kAXErrorSuccess:
        _setError(err, f'Error performing action {action}')