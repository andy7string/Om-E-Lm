...
def get_keycode(key):
    """
    Given a symbolic key (e.g., RETURN, TAB), return the integer keycode using specialKeys dict.
    If the key is already an int, return as is.
    """
    if isinstance(key, int):
        return key
    return specialKeys.get(key, None)
