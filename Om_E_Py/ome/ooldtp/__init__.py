...
class Transport(xmlrpclib.Transport):
    ...
    def _spawn_daemon(self):
        ...
        if _ldtp_windows_env:
            ...
            pycmd = 'import ome.ldtpd; ome.ldtpd.main(parentpid=%s)' % pid
    ...
    def send_request(self, connection, handler, request_body, debug):
        ...
        # The rest is handled by send_host, send_user_agent, send_content, etc.
    ...
    def request(self, host, handler, request_body, verbose=0):
        ...
        while True:
            try:
                ...
                self.send_request(h, handler, request_body, verbose)
...
class ooldtp:
    """
    Remote wrappers for text, process, and mouse event actions.
    Provides methods to interact with remote objects and perform automation tasks.
    """
    def __init__(self, server='localhost', port=4118):
        ...
        _thread.start_new_thread(self._pollEvents.run, ())
        self._pollLogs = PollLogs(self)
        _thread.start_new_thread(self._pollLogs.run, ())
            ...
            if (event_type == "onwindowcreate" and \
                ...
                try:
                    if len(args) and args[0]:
                        ...
                        _thread.start_new_thread(callback, args)
                    else:
                        # Spawn a new thread, for each event
